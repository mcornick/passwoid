...
const defaultPool = function(length: number) {
...
const randomCharacter = function(pool: string[]) {
...
const acceptable = function(pw: string) {
  return pw.match(/[A-Z]/) && pw.match(/[a-z]/) && pw.match(/\d/);
};

const generatePassword = function(pool: string[], length: number) {
  const results = [];
...
module.exports = function(length: number) {
